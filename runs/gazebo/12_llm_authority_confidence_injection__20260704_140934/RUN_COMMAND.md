# Exact PX4/Gazebo run command — S8L (12_llm_authority_confidence_injection)
...
python run_experiment.py --scenario S8L --backend px4
```

Orchestrated end-to-end (clean restart + launch + run + frame capture) via:

```bash
python -m tools.run_px4_validation --scenario S8L --vehicle gz_x500_depth
```

- LLM model: `qwen2.5:7b`
- Vehicle: `gz_x500_depth` (x500_depth)  ·  World: `nfz_restricted_zone`  ·  GUI: visible (DISPLAY=:1)
- Run folder: `runs/gazebo/12_llm_authority_confidence_injection__20260704_140934`
