# Exact PX4/Gazebo run command — S3L (07_llm_runtime_poison_behind_nfz)
...
python run_experiment.py --scenario S3L --backend px4
```

Orchestrated end-to-end (clean restart + launch + run + frame capture) via:

```bash
python -m tools.run_px4_validation --scenario S3L --vehicle gz_x500_depth
```

- LLM model: `qwen2.5:7b`
- Vehicle: `gz_x500_depth` (x500_depth)  ·  World: `nfz_restricted_zone`  ·  GUI: visible (DISPLAY=:1)
- Run folder: `runs/gazebo/07_llm_runtime_poison_behind_nfz__20260704_140638`
